...
public class ModuleServiceImpl implements ModuleService {
    ...
    public void delete(Long courseId, Long moduleId) {
        ...
        // Check for lectures (cascade behavior: block if lectures exist)
        // NOTE: This assumes LectureRepository exists. Implementation depends on Lecture entity.
        // For now, we log the deletion. When Lecture entity is ready:
        ...
        moduleRepository.softDeleteById(moduleId, LocalDateTime.now());
    ...
    public PagedResponse<ModuleResponse> reorder(Long courseId, ReorderModulesRequest request) {
        ...
        for (int i = 0; i < moduleIds.size(); i++) {
            Module module = modules.stream()
                    .filter(m -> m.getId().equals(moduleIds.get(i)))
                    .findFirst()
                    .orElseThrow();
